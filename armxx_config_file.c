/*
* Function: This module is used to read the configuration file from /etc/Module.ini
*           like, IP ADDRESS, Number of sensor node.
* Author  : Charlie Chen
* Time    : 2016/July/4
* Version : v1.0
*/
...
int 
WritePrivateProfileString(char *lpAppName,char *lpKeyName,char *lpString,char *lpFileName)
{
	char *token;
	FILE *stream;
...
int 
GetPrivateProfileString(char *lpAppName, char *lpKeyName,char *lpDefault,char *lpReturnedString,unsigned long nSize, char *lpFileName)
{
	char *token;
        FILE *stream;
        char szBuffer[MAX_STRING_LEN];
...
int 
GetPrivateProfileUL(char *lpAppName,  char *lpKeyName,unsigned long nDefault, char *lpFileName)
{
		char *token;
        FILE *stream;
        char szBuffer[MAX_STRING_LEN];
